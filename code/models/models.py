...
class GradConCAEEncoder(torch.nn.Module):
    def __init__(self, fin=1, zdim=128, dense=False, variational=False, n_blocks=4, spatial_dim=7,
                 gap=False):
        super(GradConCAEEncoder, self).__init__()
        self.fin = fin
        self.zdim = zdim
        self.dense = dense
        self.n_blocks = n_blocks
        ...
        # 1) Feature extraction
        self.backbone = torch.nn.Sequential(GradConCAEDownBlock(1, 32),
                                            GradConCAEDownBlock(32, 32),
                                            GradConCAEDownBlock(32, 64),
                                            GradConCAEDownBlock(64, 64))
        # 2) Latent space (dense or spatial)
        if self.dense:  # dense
            if gap:
                if self.variational:
                    self.mu = torch.nn.Conv2d(64, zdim, (1, 1))
                    self.log_var = torch.nn.Conv2d(64, zdim, (1, 1))
                else:
                    self.z = torch.nn.Conv2d(64, zdim, (1, 1))
            else:
                if self.variational:
                    self.mu = torch.nn.Linear(64*spatial_dim**2, zdim)
                    self.log_var = torch.nn.Linear(64*spatial_dim**2, zdim)
                else:
                    self.z = torch.nn.Linear(64 * spatial_dim ** 2, zdim)
        else:  # spatial
            if self.variational:
                self.mu = torch.nn.Conv2d(64, zdim, (1, 1))
                self.log_var = torch.nn.Conv2d(64, zdim, (1, 1))
    ...
    def forward(self, x):
        ...
        return z, z_mu, z_logvar, None


class GradConCAEDecoder(torch.nn.Module):

    def __init__(self, fin=256, nf0=128, n_channels=1, dense=False, n_blocks=4, spatial_dim=7,
                 gap=False):
        super(GradConCAEDecoder, self).__init__()
        self.n_blocks = n_blocks
        self.dense = dense
        self.spatial_dim = spatial_dim
        self.fin = fin
        ...
        if self.dense and not self.gap:
            self.dense_layer = torch.nn.Sequential(torch.nn.Linear(fin, 64*spatial_dim**2))
        if not dense:
            self.dense_layer = torch.nn.Sequential(torch.nn.Conv2d(fin, 64, (1, 1)))

        # Set number of input and output channels
        n_filters_in = [64, 64, 32, 32]
        n_filters_out = [64, 32, 32] + [n_channels]

        self.blocks = torch.nn.ModuleList()
        for i in np.arange(0, 4):
            if i == 0:
                self.blocks.append(torch.nn.Sequential(GradConCAEUpBlock(n_filters_in[i], n_filters_out[i], padding=2)))
            else:
                self.blocks.append(torch.nn.Sequential(GradConCAEUpBlock(n_filters_in[i], n_filters_out[i], padding=1)))
...
class GradConCAEDownBlock(torch.nn.Module):
    def __init__(self, in_channel, out_channel, stride=2, padding=2):
        super(GradConCAEDownBlock, self).__init__()

        self.conv1 = torch.nn.Conv2d(in_channel, out_channel, (4, 4), stride=stride, padding=padding)
...
class GradConCAEUpBlock(torch.nn.Module):
    def __init__(self, in_channel, out_channel, stride=2, padding=2):
        super(GradConCAEUpBlock, self).__init__()

        self.conv1 = torch.nn.ConvTranspose2d(in_channel, out_channel, (4, 4), stride=stride, padding=padding)
        self.act1 = torch.nn.ReLU()
