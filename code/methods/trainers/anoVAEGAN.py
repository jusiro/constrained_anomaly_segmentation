...
class AnomalyDetectorAnoVAEGAN:
    def __init__(self, dir_results, item=['flair'], zdim=32, lr=1*1e-4, input_shape=(1, 224, 224), epochs_to_test=25,
                 ...
                 loss_reconstruction='bce', restoration=False, alpha_kl=1, alpha_adversial=1):
    ...
    def train(self, train_generator, epochs, dataset_test):
        ...
        for self.i_epoch in range(self.epochs):
            ...
            for self.i_iteration, (x_n, y_n, _, _) in enumerate(self.train_generator):
                ...
                # Init overall losses
                L = self.lr_iteration + self.alpha_kl * self.kl_iteration + self.alpha_adversial * self.ladv_iteration
    ...
    def display_losses(self, on_epoch_end=False):
        ...
        # Init losses display
        info += "Reconstruction={:.4f} || KL={:.4f} || ladv={:.4f} || ldisc={:.4f}".format(lr, lkl, ladv, ldisc)
        # Print losses
    ...
    def plot_learning_curves(self):
        ...
        plot_subplot(axes[1, 1], np.arange(self.i_epoch + 1) + 1, np.array(self.ldisc_lc), "Discriminator loss")
