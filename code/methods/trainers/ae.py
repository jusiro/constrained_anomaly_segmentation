...
class AnomalyDetectorAE:
    def __init__(self, dir_results, item=['flair'], zdim=32, lr=1*1e-4, input_shape=(1, 224, 224), epochs_to_test=25,
                 ...
                 loss_reconstruction='bce', restoration=False):
    ...
    def train(self, train_generator, epochs, dataset_test):
        ...
        for self.i_epoch in range(self.epochs):
            ...
            for self.i_iteration, (x_n, y_n, _, _) in enumerate(self.train_generator):
                ...
                # Calculate criterion
                self.lr_iteration = self.Lr(xhat, x_n) / (self.train_generator.batch_size * self.input_shape[1] *
                                                          self.input_shape[2])  # Reconstruction loss
    ...
    def on_epoch_end(self):
        ...
        if (self.i_epoch + 1) % self.epochs_to_test == 0:
            ...
            history = pd.DataFrame(list(zip(self.lr_lc, self.aucroc_lc, self.auprc_lc)),
                                   columns=['Lrec', 'AUCROC', 'AUPRC'])
    ...
    def restoration_reconstruction(self, x):
        ...
        for i in np.arange(N):
            ...
            L = lr / (self.input[1] * self.input[2])
    ...
    def display_losses(self, on_epoch_end=False):
        ...
        # Init losses display
        info += "Reconstruction={:.4f}".format(lr)
        # Print losses
    ...
    def plot_learning_curves(self):
        ...
        plot_subplot(axes, np.arange(self.i_epoch + 1) + 1, np.array(self.lr_lc), "Reconstruc loss")
